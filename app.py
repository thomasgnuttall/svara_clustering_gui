...
clus_method_header = html.Div([html.H4(id="cluster-text",children="Clustering method (for when individual svaras are selected)", style={'font-family':'sans-serif'})], style={'display':'flex', 'height':45})
clus_method_dd = dcc.Dropdown(
           id = 'clustering-method',
           value='kmeans',
           placeholder='Select clustering method',
           options= [
               {'label' : 'K-Means', 'value' : 'kmeans'},
               {'label' : 'HDBSCAN', 'value' : 'hdbscan'}], style={'width':500, 'font-family':'sans-serif'})
data_header = html.Div([html.H4(id="data-input-text",children="Input data", style={'font-family':'sans-serif'})], style={'display':'flex', 'height':45})
data_dd = dcc.Dropdown(
           id = 'data-type',
           value='emb',
           placeholder='Select input data',
           options= [
               {'label' : 'Embeddings + static', 'value' : 'embstat'},
               {'label' : 'Embeddings only', 'value' : 'emb'},
               {'label' : 'Static only', 'value' : 'stat'}], style={'width':500, 'font-family':'sans-serif'})
...
[
    dbc.Row([dbc.Col([svara_header, svara_dd, clus_method_header, clus_method_dd, data_header, data_dd, graph]), dbc.Col([dbc.Row([clus_header, clus_dd]), dbc.Row([timeline_img]), dbc.Row([svara_img]), dbc.Row([features_img])])], style={'display':'flex'}),
    tooltip,
    placeholder
])
...
@app.callback(
    ...
    [Input('input-data', 'value'), Input('clustering-method', 'value'), Input('data-type', 'value')]
)
def choose_plot_data(input_data, clustering_method, data_type):
    ...
    if input_data is None:
        raise dash.exceptions.PreventUpdate()
    if input_data == 'Sa':
        data_path = f'data/analysis/{analysis_name}/clusters/{data_type}/{clustering_method}/embedding_kmeans_sa.csv'
        this_direc = f'data/analysis/{analysis_name}/clusters/{data_type}/{clustering_method}/sa/'
    if input_data == 'Ri':
        data_path = f'data/analysis/{analysis_name}/clusters/{data_type}/{clustering_method}/embedding_kmeans_ri.csv'
        this_direc = f'data/analysis/{analysis_name}/clusters/{data_type}/{clustering_method}/ri/'
    if input_data == 'Ga':
        data_path = f'data/analysis/{analysis_name}/clusters/{data_type}/{clustering_method}/embedding_kmeans_ga.csv'
        this_direc = f'data/analysis/{analysis_name}/clusters/{data_type}/{clustering_method}/ga/'
    if input_data == 'Ma':
        data_path = f'data/analysis/{analysis_name}/clusters/{data_type}/{clustering_method}/embedding_kmeans_ma.csv'
        this_direc = f'data/analysis/{analysis_name}/clusters/{data_type}/{clustering_method}/ma/'
    if input_data == 'Pa':
        data_path = f'data/analysis/{analysis_name}/clusters/{data_type}/{clustering_method}/embedding_kmeans_pa.csv'
        this_direc = f'data/analysis/{analysis_name}/clusters/{data_type}/{clustering_method}/pa/'
    if input_data == 'Dha':
        data_path = f'data/analysis/{analysis_name}/clusters/{data_type}/{clustering_method}/embedding_kmeans_dha.csv'
        this_direc = f'data/analysis/{analysis_name}/clusters/{data_type}/{clustering_method}/dha/'
    if input_data == 'Ni':
        data_path = f'data/analysis/{analysis_name}/clusters/{data_type}/{clustering_method}/embedding_kmeans_ni.csv'
        this_direc = f'data/analysis/{analysis_name}/clusters/{data_type}/{clustering_method}/ni/'
    if input_data == 'All':
        data_path = f'data/analysis/{analysis_name}/clusters/{data_type}/embedding_umap.csv'
        this_direc = ''
        disab = True
